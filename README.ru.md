...
При установке вы задаёте **пароль панели**: он защищает панель *и* шифрует сессию. Ключ **никогда не записывается на диск** — он выводится из пароля при каждом запуске. Всё хранится в одной папке `config/`. **Забыли пароль — восстановления нет.** Для автоматического запуска: `ATERKEEP_KEY='ваш-пароль' ./aterkeep`
